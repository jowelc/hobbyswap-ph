    ...
    <div className="min-h-screen bg-slate-950 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8 space-y-8">
        ...
        <div>
          ...
          <TradesPanel onDoneDeal={() => {
            fetch('/api/items/mine').then((r) => r.json()).then((d) => setItems(Array.isArray(d) ? d : [])).catch(() => {});
          }} />
        </div>
      ...
      <div className="p-2.5 flex flex-col gap-1.5 flex-1">
        ...
        {(item.watcherCount ?? 0) > 0 && (
          <div className="flex items-center gap-1 text-[10px] text-amber-400/80">
            ...
            <span>
              {item.watcherCount} {item.watcherCount === 1 ? 'person' : 'people'} watching
            </span>
          </div>
        )}
      </div>
